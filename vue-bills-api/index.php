...
/***
 * Listar Lançamentos por tipo:
 * 1 = Pay
 * 2 = Receive
 */
$app->get('api/bills/type/{type}', function ($type) use ($app) {
    $bills_ = getBills();
    $bills = [];
    foreach ($bills_ as $key => $bill) {
        if ($bill['type'] == $type) {
 ...
 * Total por Tipo
 */
$app->get('api/bills/total/{type}', function ($type) use ($app) {
    $bills_ = getBills();
    $bills  = [];
    foreach ($bills_ as $key => $bill) {
        if ($bill['type'] == $type) {
